    ...
    [
        ...
        [
            tables.profile.findAll( {
                attributes: [ 'profileId', 'personalWeb', 'email', 'position' ],
            } ),
            ...
            tables.title.findAll( {
                attributes: [ 'profileId', 'titleId' ],
            } ),
            tables.titleI18n.findAll( {
                attributes: [ 'titleId', 'title' ],
                where: { language },
            } ),
            tables.department.findAll( {
                attributes: [ 'profileId', 'departmentId' ],
            } ),
            tables.departmentI18n.findAll( {
                attributes: ['departmentId', 'department'],
                where: { language, },
            } ),
            tables.office.findAll( {
                attributes: ['profileId', 'officeId', 'tel'],
            } ),
            tables.officeI18n.findAll( {
                attributes: ['officeId', 'address'],
                where: { language },
            } ),
        ]
    );
