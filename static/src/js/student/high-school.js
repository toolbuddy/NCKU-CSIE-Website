...
d3Data.lineChart.forEach( ( chart ) => {
    const xScale = d3.scaleLinear( [ 0,
        chart.data.length, ], [ 0,
        d3Data.plotAttribute.width, ] );
    const yScale = d3.scaleLinear( [ chart.yMin,
        chart.yMax, ], [ d3Data.plotAttribute.height,
        0, ] );

    const scaling = d3.line().x( ( d, i ) => xScale( i + 1 ) ).y( d => yScale( d ) );
    ...
    const xAxis = d3.axisBottom( xScale ).tickSize( d3Data.plotAttribute.height * 0.8 ).tickFormat( d => 105 + d - 1 ).tickValues( tickValuesXAxis );
