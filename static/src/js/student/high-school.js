...
d3Data.lineChart.forEach(chart => {
    const xScale = d3.scaleLinear( [ 0, chart.data.length ], [ 0, d3Data.plotAttribute.width ] );
    const yScale = d3.scaleLinear( [ chart.yMin, chart.yMax ], [ d3Data.plotAttribute.height, 0 ] );

    const scaling = d3.line().x( (d,i) => { 
            return xScale( i + 1 );
        }).y( ( d ) => { 
            return yScale( d );
        });
    ...
    d3.select( `#${ chart.selector }` ).select( 'svg' )
        ...
        .attr( 'viewBox', `0 0 ${ d3Data.plotAttribute.width + 2*d3Data.plotAttribute.margin } ${ d3Data.plotAttribute.height + 2*d3Data.plotAttribute.margin }` )
    ...
    var xAxis = d3.axisBottom(xScale).tickSize(d3Data.plotAttribute.height*0.8).tickFormat(d => 105+d-1).tickValues(tickValuesXAxis);
    d3.select( `#${ chart.selector }` ).select( 'svg' ).append( 'g' )
        .attr("class", "lineChart__axis")
        .attr("transform", `translate(-${ d3Data.plotAttribute.margin }, -${ d3Data.plotAttribute.margin })`)
        .call(xAxis);

    // create x-axis labels
    d3.select( `#${ chart.selector }` ).select( 'svg' ).append("text")
    ...
    // create data labels
    d3.select( `#${ chart.selector }` ).select( 'svg' ).selectAll(".text")  		
        .data(chart.data)
        .enter()
        .append("text")
